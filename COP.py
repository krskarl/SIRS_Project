...
group_server_url = None  # URL of the separate Group Management Server
...
def share_with_group(transaction_id):
    """Share transaction with a group - queries the separate Group Server"""
    data = request.json
    ...
    # Query the separate Group Server for group members
    try:
        print(f"[MAIN SERVER] Querying Group Server for group '{group_name}'...")
        response = requests.get(f"{group_server_url}/api/groups/{group_name}", verify=False)
        group_data = response.json()

        if not group_data.get('success'):
            return jsonify({'success': False, 'error': f"Group Server: {group_data.get('error', 'Group not found')}"}), 404

        members = group_data.get('members', [])
        print(f"[MAIN SERVER] Group Server returned members: {members}")

    except Exception as e:
        return jsonify({'success': False, 'error': f'Failed to contact Group Server: {str(e)}'}), 503
    ...
    db.update_transaction(transaction_id, protected_doc)
    return jsonify({'success': True, 'message': f'Shared with group {group_name} ({len(members)} members)'})
...
def main():
    ...
    if mode == 'help':
        ...
        grp_server = os.environ.get('COP_GROUP_SERVER', 'https://localhost:8001')
        ...
        if len(sys.argv) > 4:
            db_host = sys.argv[4]
        if len(sys.argv) > 5:
            grp_server = sys.argv[5]

        # Initialize database connection with TLS
        global db, group_server_url
        group_server_url = grp_server
        ...
        print(f"[SERVER] Group Server URL: {group_server_url}")
...
SERVER/CLIENT COMMANDS:
  ...
  python COP.py server [port] [use_tls] [db_host] [group_server_url]
      Start the Main CoP server
      Example: python COP.py server 8000 true 10.0.1.10 https://10.0.1.20:8001
      ...
      Environment variables:
        ...
        COP_GROUP_SERVER  - Group Server URL (default: https://localhost:8001)

  python GroupServer.py [port] [use_tls] [db_host]
      Start the separate Group Management Server (Security Challenge A)
      Example: python GroupServer.py 8001 true 10.0.1.10
...
ARCHITECTURE (Security Challenge A):

  Client --> Main Server (port 8000) --> Database
                  |
                  +--> Group Server (port 8001) --> Database

  The Group Server is a SEPARATE server that manages groups and members.
  When sharing with a group, the Main Server queries the Group Server.
""")
